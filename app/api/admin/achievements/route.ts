...
export async function POST(req: NextRequest) {
  ...
  const { data, error } = await supabaseAdmin.from('achievements').insert(body).select().single()
  if (error) return NextResponse.json({ error: error.message }, { status: 500 })
  revalidatePath('/thanh-tich')
  revalidatePath('/api/public/achievements')
...
export async function PUT(req: NextRequest) {
  ...
  const { data, error } = await supabaseAdmin.from('achievements').update(fields).eq('id', id).select().single()
  if (error) return NextResponse.json({ error: error.message }, { status: 500 })
  revalidatePath('/thanh-tich')
  revalidatePath('/api/public/achievements')
  return NextResponse.json(data)
